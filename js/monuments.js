...
const filterToMapViewBtnLabel = document.getElementById("filterToMapViewBtnLabel");

const drawMonumentSpatialPolygonBtn = document.getElementById(
  "drawMonumentSpatialPolygonBtn"
);
const drawMonumentSpatialPolygonBtnLabel = document.getElementById(
  "drawMonumentSpatialPolygonBtnLabel"
);
const monumentSpatialDrawMessage = document.getElementById(
  "monumentSpatialDrawMessage"
);
const monumentSpatialDrawMessageText = document.getElementById(
  "monumentSpatialDrawMessageText"
);
const cancelMonumentSpatialDrawBtn = document.getElementById(
...
const closeMonumentMeasurementPanelBtn = document.getElementById(
  "closeMonumentMeasurementPanelBtn"
);
const monumentMeasureDistanceBtn = document.getElementById(
  "monumentMeasureDistanceBtn"
);
const monumentMeasureAreaBtn = document.getElementById(
  "monumentMeasureAreaBtn"
);
const monumentMeasurementInstruction = document.getElementById(
  "monumentMeasurementInstruction"
);
...
let monumentResourceSearchAbortController = null;
let monumentResourceSearchRecords = [];
let monumentResourceSearchPanel = null;
...
let monumentSpatialDrawCoordinates = [];

let monumentMeasurementControlBtn = null;
let monumentMeasurementMode = null;
let monumentMeasurementCoordinates = [];
let monumentMeasurementIsActive = false;
let monumentMeasurementEventsBound = false;

const MONUMENT_MEASUREMENT_SOURCE_ID = "monument-measurement-source";
const MONUMENT_MEASUREMENT_LINE_LAYER_ID = "monument-measurement-line";
const MONUMENT_MEASUREMENT_FILL_LAYER_ID = "monument-measurement-fill";
const MONUMENT_MEASUREMENT_POINT_LAYER_ID = "monument-measurement-points";
...
async function downloadMonumentsMapImage() {
  ...
  // Scale bar
  const scale = calculateMapScaleBar(map);
  ...
  link.download = `caal-distribution-map-${new Date().toISOString().slice(0, 10)}.png`;
  link.click();
}
...
function monumentMapInteractionToolIsActive() {
...
function initialiseMonumentSpatialDraw() {
  ...
  if (!terraDrawLib || !mapLibreAdapterLib) {
    ...
    if (drawMonumentSpatialPolygonBtn) {
      drawMonumentSpatialPolygonBtn.disabled = true;
      drawMonumentSpatialPolygonBtn.title = t(
        "viewer_spatial_polygon_unavailable",
  ...
  const { TerraDrawMapLibreGLAdapter } = mapLibreAdapterLib;

  monumentSpatialDraw = new TerraDraw({
    ...
    modes: [
      ...
      new TerraDrawRenderMode({
        modeName: "monument-spatial-render",
        styles: {
  ...
  monumentSpatialDraw.start();
  monumentSpatialDraw.setMode("monument-spatial-render");

  monumentSpatialDraw.on("finish", async (featureId, context) => {
    if (
    ...
    const snapshot = monumentSpatialDraw.getSnapshot();
  ...
  map.on("click", (event) => {
    ...
    const previous =
      monumentSpatialDrawCoordinates[
        monumentSpatialDrawCoordinates.length - 1
      ];
    ...
    monumentSpatialDrawCoordinates.push(coordinate);
  });
  ...
  map.on("contextmenu", (event) => {
    ...
    void finishMonumentSpatialPolygonWithRightClick();
  });
}
...
async function finishMonumentSpatialPolygonWithRightClick() {
  ...
  if (monumentSpatialDrawCoordinates.length < 3) {
    showMonumentMapToolMessage(
  ...
  const firstCoordinate = monumentSpatialDrawCoordinates[0];
  const ring = [
    ...monumentSpatialDrawCoordinates.map((coordinate) => [
      coordinate[0],
      coordinate[1]
    ]),
  ...
  const geometry = normaliseMonumentSpatialPolygon({
    type: "Polygon",
    coordinates: [ring]
  });
  ...
  const featureId =
    ...
    `monument-spatial-${Date.now()}-${Math.random()
      .toString(36)
      .slice(2)}`;

  const addResult = monumentSpatialDraw.addFeatures([
    {
      id: featureId,
      type: "Feature",
      properties: {
        mode: "monument-spatial-render"
      },
      geometry
    }
  ])?.[0];
...
function normaliseMonumentSpatialPolygon(geometry) {
  ...
  for (const ring of rings) {
    ...
    for (const coordinate of ring) {
      ...
      totalVertices += 1;
      if (totalVertices > 200) return null;
    }
...
function removeMonumentSpatialDrawFeature() {
  if (!monumentSpatialDraw || monumentSpatialDrawFeatureId === null) {
    monumentSpatialDrawFeatureId = null;
    return;
  }

  try {
    monumentSpatialDraw.removeFeatures([
      monumentSpatialDrawFeatureId
    ]);
  } catch (error) {
    console.warn("Could not remove monument spatial polygon:", error);
  }

  monumentSpatialDrawFeatureId = null;
}

function setMonumentSpatialDrawMessage(visible) {
  if (!monumentSpatialDrawMessage) return;
...
function updateMonumentSpatialPolygonButton() {
  if (!drawMonumentSpatialPolygonBtn) return;

  const isApplied = Boolean(activeMonumentSpatialPolygon);

  if (drawMonumentSpatialPolygonBtnLabel) {
    drawMonumentSpatialPolygonBtnLabel.textContent = monumentSpatialDrawIsActive
  ...
  drawMonumentSpatialPolygonBtn.setAttribute(
    "aria-pressed",
    isApplied || monumentSpatialDrawIsActive ? "true" : "false"
  );

  drawMonumentSpatialPolygonBtn.classList.toggle(
    "is-active",
    isApplied || monumentSpatialDrawIsActive
  );
}
...
function cancelMonumentSpatialPolygonDrawing({
...
async function clearMonumentSpatialPolygonFilter({
  reload = true
} = {}) {
  cancelMonumentSpatialPolygonDrawing({
    clearCompletedPolygon: true
...
function addMonumentMeasurementControl() {
  ...
  class MonumentMeasurementControl {
    onAdd(mapInstance) {
      this._map = mapInstance;
      ...
      const button = document.createElement("button");
      const label = t("measurement_tools", "Measurement tools");

      button.type = "button";
      button.id = "monumentMeasurementControlBtn";
      button.className =
      ...
      button.addEventListener("click", (event) => {
        event.preventDefault();
        event.stopPropagation();
        toggleMonumentMeasurementPanel();
      });

      container.appendChild(button);
      monumentMeasurementControlBtn = button;
      this._container = container;
    ...
    onRemove() {
      ...
      monumentMeasurementControlBtn = null;
      this._map = null;
    }
  }
...
function monumentDistanceMetres(a, b) {
  if (!Array.isArray(a) || !Array.isArray(b)) return 0;

  const earthRadiusMetres = 6371008.8;
  const lon1 = monumentDegreesToRadians(a[0]);
  const lat1 = monumentDegreesToRadians(a[1]);
  const lon2 = monumentDegreesToRadians(b[0]);
  const lat2 = monumentDegreesToRadians(b[1]);
  ...
  return 2 * earthRadiusMetres * Math.atan2(
    Math.sqrt(haversine),
    Math.sqrt(1 - haversine)
  );
}

function monumentMeasurementTotalDistanceMetres(coordinates = []) {
  let total = 0;

  for (let index = 1; index < coordinates.length; index += 1) {
    total += monumentDistanceMetres(
      coordinates[index - 1],
      coordinates[index]
    );
  }
...
function monumentPolygonAreaSquareMetres(coordinates = []) {
  ...
  for (let index = 0; index < coordinates.length; index += 1) {
    ...
    const lon1 = monumentDegreesToRadians(current[0]);
    const lon2 = monumentDegreesToRadians(next[0]);
    const lat1 = monumentDegreesToRadians(current[1]);
    const lat2 = monumentDegreesToRadians(next[1]);
...
function monumentFormatDistance(metres) {
...
function monumentFormatArea(squareMetres) {
...
function monumentMeasurementGeojson() {
  const coordinates = monumentMeasurementCoordinates;
  const features = [];

  if (coordinates.length) {
    features.push({
      type: "Feature",
      properties: { measurement_geometry: "points" },
      geometry: {
        type: "MultiPoint",
        coordinates
      }
  ...
  if (coordinates.length >= 2) {
    features.push({
      type: "Feature",
      properties: { measurement_geometry: "line" },
      geometry: {
        type: "LineString",
        coordinates
      }
  ...
  if (
    monumentMeasurementMode === "area" &&
    coordinates.length >= 3
  ) {
    features.push({
      type: "Feature",
      properties: { measurement_geometry: "area" },
      geometry: {
        type: "Polygon",
...
function ensureMonumentMeasurementLayers() {
  if (!map || !mapLoaded) return;

  if (!map.getSource(MONUMENT_MEASUREMENT_SOURCE_ID)) {
    map.addSource(MONUMENT_MEASUREMENT_SOURCE_ID, {
      type: "geojson",
      data: monumentMeasurementGeojson()
    });
  }

  if (!map.getLayer(MONUMENT_MEASUREMENT_FILL_LAYER_ID)) {
    map.addLayer({
      id: MONUMENT_MEASUREMENT_FILL_LAYER_ID,
      type: "fill",
      source: MONUMENT_MEASUREMENT_SOURCE_ID,
      filter: ["==", ["get", "measurement_geometry"], "area"],
      paint: {
        "fill-color": "#22D3EE",
        "fill-opacity": 0.22
      }
  ...
  if (!map.getLayer(MONUMENT_MEASUREMENT_LINE_LAYER_ID)) {
    map.addLayer({
      id: MONUMENT_MEASUREMENT_LINE_LAYER_ID,
      type: "line",
      source: MONUMENT_MEASUREMENT_SOURCE_ID,
      filter: [
        "any",
        ["==", ["get", "measurement_geometry"], "line"],
        ["==", ["get", "measurement_geometry"], "area"]
      ],
      paint: {
        "line-color": "#083344",
        "line-width": 3,
  ...
  if (!map.getLayer(MONUMENT_MEASUREMENT_POINT_LAYER_ID)) {
    map.addLayer({
      id: MONUMENT_MEASUREMENT_POINT_LAYER_ID,
      type: "circle",
      source: MONUMENT_MEASUREMENT_SOURCE_ID,
      filter: ["==", ["get", "measurement_geometry"], "points"],
      paint: {
        "circle-radius": 5,
        "circle-color": "#22D3EE",
...
function updateMonumentMeasurementLayers() {
  ensureMonumentMeasurementLayers();

  map?.getSource(MONUMENT_MEASUREMENT_SOURCE_ID)
    ?.setData?.(monumentMeasurementGeojson());

  [
    MONUMENT_MEASUREMENT_FILL_LAYER_ID,
    MONUMENT_MEASUREMENT_LINE_LAYER_ID,
    MONUMENT_MEASUREMENT_POINT_LAYER_ID
  ].forEach((layerId) => {
...
function updateMonumentMeasurementResult() {
  if (!monumentMeasurementResult || !monumentMeasurementInstruction) {
    return;
  }

  const coordinateCount = monumentMeasurementCoordinates.length;
  ...
  if (monumentMeasurementMode === "distance") {
    monumentMeasurementInstruction.textContent = monumentMeasurementIsActive
      ? t(
    ...
    const distance = monumentMeasurementTotalDistanceMetres(
    ...
    monumentMeasurementResult
      .querySelector("[data-monument-clear-measurement]")
  ...
  monumentMeasurementResult.innerHTML = `
    ...
    <span>
      ...
      ${escapeHtml(monumentFormatDistance(perimeter))}
    </span>
    <button
      type="button"
      class="viewer-measurement-clear-inline"
...
function updateMonumentMeasurementButtons() {
  const distanceActive = monumentMeasurementMode === "distance";
  const areaActive = monumentMeasurementMode === "area";

  monumentMeasureDistanceBtn?.classList.toggle(
    "is-active",
    distanceActive
  );
  ...
  monumentMeasureAreaBtn?.classList.toggle(
    "is-active",
    areaActive
  );
...
function clearMonumentMeasurement({ preserveMode = false } = {}) {
  monumentMeasurementCoordinates = [];
...
function finishMonumentMeasurement() {
  if (!monumentMeasurementIsActive) return;

  const minimumPoints = monumentMeasurementMode === "area" ? 3 : 2;

  if (monumentMeasurementCoordinates.length < minimumPoints) {
    showMonumentMapToolMessage(
      monumentMeasurementMode === "area"
...
function bindMonumentMeasurementMapEvents() {
  if (!map || monumentMeasurementEventsBound) return;

  monumentMeasurementEventsBound = true;
  ...
  map.on("click", (event) => {
    ...
    const previous =
      monumentMeasurementCoordinates[
        monumentMeasurementCoordinates.length - 1
      ];
    ...
    if (
      monumentMeasurementMode === "area" &&
      monumentMeasurementCoordinates.length >= 3
    ) {
      const firstPixel = map.project(
        monumentMeasurementCoordinates[0]
      );
      const clickPixel = map.project(coordinate);
      ...
      if (
        ...
        finishMonumentMeasurement();
        return;
      }
    }

    monumentMeasurementCoordinates.push(coordinate);
    updateMonumentMeasurementLayers();
    updateMonumentMeasurementResult();
  });
  ...
  document
    .querySelectorAll(
      [
        ...
        ".result-preview-btn",
        ".result-centre-btn",
        ".monument-resource-search-card-related",
        ".monument-resource-preview-btn",
        ".related-id-chip",
...
function getActiveFilterChips() {
  ...
  if (activeMonumentSpatialPolygon) {
    chips.push({
...
function renderNoScopeSelectedState() {
  ...
  monumentRenderResourceSearchResults([]);
}
...
function scheduleMonumentSearchAndMapRedraw() {
  ...
  if (!monumentShouldRunResourceSearch()) {
    monumentRenderResourceSearchResults([]);
  }
...
function buildMonumentQueryParams({ includePaging = true } = {}) {
  ...
  if (activeMonumentSpatialPolygon) {
    params.set(
      "spatialPolygon",
      JSON.stringify(activeMonumentSpatialPolygon)
    );
...
function monumentResourceSearchQuery() {
  return String(siteSearch?.value || "").trim();
}

function monumentShouldRunResourceSearch() {
  return monumentResourceSearchQuery().length >= 2;
}

function monumentEnsureResourceSearchPanel() {
  if (
    monumentResourceSearchPanel &&
    document.body.contains(monumentResourceSearchPanel)
  ) {
    return monumentResourceSearchPanel;
  }

  if (!resultsList) return null;

  monumentResourceSearchPanel = document.createElement("div");
  monumentResourceSearchPanel.id = "monumentResourceSearchPanel";
  monumentResourceSearchPanel.className = "monument-resource-search-panel";
  monumentResourceSearchPanel.hidden = true;

  resultsList.insertAdjacentElement("afterend", monumentResourceSearchPanel);

  return monumentResourceSearchPanel;
}

function monumentSafeSearchText(value, fallback = "") {
  ...
  return mAttributeValue(text);
}

function monumentResourceTypeLabel(record) {
  const type = String(record?.record_type || "").trim();

  switch (type) {
    case "archive":
      return mLabel("Archive", "Archive");

    case "monument":
      return mLabel("Monuments", "Monuments");

    case "institution":
      return mLabel("Institutions", "Institutions");

    case "dataset":
      return mLabel("Datasets", "Datasets");

    case "rs3_poly":
      return mLabel("RS3 polygons", "RS3 polygons");

    case "rs3_line":
      return mLabel("RS3 lines", "RS3 lines");

    case "rs3_group":
      return mLabel("RS3 groups", "RS3 groups");

    case "vernacular":
      return mLabel("Vernacular", "Vernacular");
...
function monumentTruncateText(value, maxLength = 55) {
  const text = String(value || "").trim();
...
function monumentRelatedToLine(record) {
  const relatedId = String(record?.matched_related_caal_id || "").trim();

  if (!relatedId) return "";

  const relatedLabel = monumentTruncateText(
  ...
  return `
    <div class="monument-resource-related-line">
      ${t("related_to", "Related to")} ${monumentSafeSearchText(relatedText)}
    </div>
  `;
}

function monumentRelatedMatchKey(record) {
  return [
...
function monumentResourceResultKey(record) {
  return [
    record?.record_type || "",
    record?.caal_id || ""
  ].join("::").toLowerCase();
...
function monumentCollapseRelatedRecords(rows) {
  const byMonumentId = new Map();

  rows.forEach((record) => {
    const key = String(record?.caal_id || "").trim().toLowerCase();
    if (!key) return;

    if (!byMonumentId.has(key)) {
      byMonumentId.set(key, {
        ...record,
        _relatedMatches: []
      });
    }

    const collapsed = byMonumentId.get(key);
    const matchKey = monumentRelatedMatchKey(record);

    const alreadyHasMatch = collapsed._relatedMatches.some((match) => {
      return monumentRelatedMatchKey(match) === matchKey;
    });
  ...
  return Array.from(byMonumentId.values());
}

function monumentCollapsePreviewOnlyRecords(rows) {
  const byResource = new Map();

  rows.forEach((record) => {
    const key = monumentResourceResultKey(record);
...
function monumentRelatedToLines(record, { limit = 3 } = {}) {
  ...
  const lines = visibleMatches.map((match) => {
    return monumentRelatedToLine(match);
  }).join("");
  ...
  return `
    ${lines}
    <div class="monument-resource-related-more">
      ${t("more_related_matches", "+ {count} more related matches").replace("{count}", remaining)}
    </div>
  `;
}

function monumentGroupResourceSearchRecords(records) {
  ...
  const nativeRelatedRows = rows.filter((record) => {
    return record.record_type === "monument" && record.match_type === "related";
  });

  const otherPreviewRows = rows.filter((record) => {
    if (record.record_type === "monument") return false;
  ...
  return {
    nativeRelated: monumentCollapseRelatedRecords(nativeRelatedRows),
    otherPreview: monumentCollapsePreviewOnlyRecords(otherPreviewRows)
  };
}

async function monumentFetchResourceSearchResults(query) {
  if (monumentResourceSearchAbortController) {
    monumentResourceSearchAbortController.abort();
  }

  monumentResourceSearchAbortController = new AbortController();
  ...
  const response = await fetch(`/api/search/resources?${params.toString()}`, {
    method: "GET",
    credentials: "include",
    signal: monumentResourceSearchAbortController.signal
  });
...
async function monumentLoadResourceSearchResults() {
  const panel = monumentEnsureResourceSearchPanel();

  if (!panel) return;

  const query = monumentResourceSearchQuery();
  ...
  panel.hidden = false;
  panel.innerHTML = `
    <div class="monument-resource-search-loading">
      <span class="mini-spinner"></span>
      ${t("searching_related_caal_records", "Searching related CAAL records...")}
    </div>
  `;
  ...
  try {
    ...
    console.warn("Monument resource search failed:", error);

    panel.hidden = false;
    panel.innerHTML = `
      <div class="monument-resource-search-error">
...
function monumentRenderNativeRelatedResourceCard(record, index) {
  const relatedLines = monumentRelatedToLines(record, { limit: 3 });
  ...
  return `
    <div
      class="monument-resource-search-card monument-resource-search-card-related"
      data-resource-search-index="${resourceIndex}"
      role="button"
      tabindex="0"
    >
      <div class="monument-resource-card-title">
        ${monumentSafeSearchText(record.display_label || record.caal_id)}
      </div>

      <div class="monument-resource-card-id">
        ${monumentSafeSearchText(record.caal_id)}
      </div>
...
function monumentRenderPreviewOnlyResourceCard(record, index) {
  const resourceIndex = Number.isInteger(record._resourceSearchIndex)
    ? record._resourceSearchIndex
    : index;

  const typeLabel = monumentResourceTypeLabel(record);
  const caalId = String(record?.caal_id || "").trim();
  const title = record?.display_label || caalId;

  return `
    <div
      class="monument-resource-search-card monument-resource-search-card-preview-only"
      data-resource-search-index="${resourceIndex}"
    >
      <div class="monument-resource-preview-row">
        <div class="monument-resource-preview-text">
          <div class="monument-resource-card-compact-line">
            <span class="monument-resource-type">${monumentSafeSearchText(typeLabel)}</span>
            <span class="monument-resource-card-id">${monumentSafeSearchText(caalId)}</span>
          </div>

          <div class="monument-resource-card-title monument-resource-card-title-compact">
            ${monumentSafeSearchText(title)}
          </div>
        </div>

        <button
          type="button"
          class="monument-resource-preview-btn"
          data-resource-search-index="${resourceIndex}"
          title="${t("preview", "Preview")}"
          aria-label="${t("record_preview", "Record preview")}: ${mAttributeValue(caalId)}"
        >
          ${svgEyeIcon()}
        </button>
      </div>
...
function monumentRenderResourceSearchSection(title, records, renderCard) {
  if (!records.length) return "";

  return `
    <section class="monument-resource-search-section">
      <h4 class="monument-resource-search-heading">
        ${title} <span class="monument-resource-search-count">(${records.length})</span>
      </h4>

      <div class="monument-resource-search-list">
...
async function monumentOpenResourceSearchMonumentRecord(record) {
  const caalId = String(record?.caal_id || "").trim();

  if (!caalId || monumentRecordOpenInProgress) return;
  ...
  try {
    ...
    if (!response.ok || !data.ok || !data.record) {
      alert(data.error || t("could_not_load_full_monument_record", "Could not load full monument record"));
      return;
    }

    if (data.record_type !== "monument") {
      await monumentPreviewResourceSearchRecord(record);
      return;
    }

    const fullRecord = mergeLiveGeometryForRecentlySavedRecord(data.record);
    ...
    console.error("Could not open resource-search monument record:", error);
    alert(
      error.message ||
...
function monumentWireResourceSearchCards() {
  const panel = monumentEnsureResourceSearchPanel();

  if (!panel) return;

  panel.querySelectorAll(".monument-resource-preview-btn").forEach((btn) => {
    ...
    btn.addEventListener("click", async (event) => {
      ...
      const record = monumentResourceSearchRecords[index];
  ...
  panel.querySelectorAll(".monument-resource-search-card-related").forEach((card) => {
    ...
    const openOrPreview = async () => {
      ...
      await monumentOpenResourceSearchMonumentRecord(record);
    };
...
function renderRelatedMonumentModal(record, caalId, fullRecordUrl) {
  ...
  monumentPreviewBody.innerHTML = `
    <div class="record-title related-record-title related-preview-title-card">
      <div class="related-preview-title-main">
        <h3>${mSafeValue(title)}</h3>
        <p>${mSafeValue(caalId)}</p>

        <div class="record-title-actions related-preview-title-actions">
          ${relatedPreviewBackButtonHtml()}
...
async function loadMonumentListRecords() {
  ...
  try {
    ...
    await monumentLoadResourceSearchResults();
  } catch (error) {
    if (requestSeq !== monumentListRequestSeq) {
      return;
    }
  ...
  const message =
    kind === "exact"
      ? t(
          ...
          "{count} records share the same mapped coordinate."
        )
      : t(
...
monumentMeasureAreaBtn?.addEventListener("click", () => {
  startMonumentMeasurement("area");
});

closeMonumentMeasurementPanelBtn?.addEventListener("click", () => {
